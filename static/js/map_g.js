        ...
        if (data.length > 0) {
            data.forEach(usuario => {
                ...
                const marker = L.marker([usuario.lat, usuario.lon])
                    .bindPopup(`${usuario.nombre} está aquí<br>Fecha turno: ${fechaFormateada}<br>Inicio del turno: ${usuario.ini}<br>Fin del turno: ${usuario.fin} <br>Tiempo Activo: ${usuario.tiempo_trabajo} <br>Estado: ${usuario.estado ? "Activo" : "Inactivo"}`);
